...
class _BollingerBandsInputScreenState extends State<BollingerBandsInputScreen> {
  ...
  Future<void> calculateBollingerBands(String symbol, String authToken) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final bollingerBandsData = jsonDecode(response.body);
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => BollingerBandsRecommendationScreen(
  ...
  void _navigateToChartScreen(String symbol) {
    if (_validateSymbol(symbol)) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => BollingerBandsChartScreen(symbol: symbol),
        ),
      );
    }
  }
  ...
  void _navigateToStrategiesScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => BollingerBandsStrategiesScreen(),
      ),
    );
  }
