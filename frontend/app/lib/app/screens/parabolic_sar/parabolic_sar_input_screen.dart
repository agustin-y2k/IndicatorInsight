// parabolic_sar_input_screen.dart
import 'package:flutter/material.dart';
import 'package:flutter_gen/gen_l10n/app_localizations.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'parabolic_sar_recommendation_screen.dart';
import 'parabolic_sar_chart_screen.dart';
import 'parabolic_sar_strategies_screen.dart';

class ParabolicSARInputScreen extends StatefulWidget {
  const ParabolicSARInputScreen({Key? key}) : super(key: key);

  @override
  _ParabolicSARInputScreenState createState() =>
      _ParabolicSARInputScreenState();
}

class _ParabolicSARInputScreenState extends State<ParabolicSARInputScreen> {
  ...
  Future<void> calculateParabolicSAR(String symbol) async {
    if (!_validateSymbol(symbol)) {
      return;
    }

    final url = Uri.parse(
        'http://179.42.171.30:12018/companies/$symbol/indicators/parabolic_sar');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final parabolicSARData = jsonDecode(response.body);
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ParabolicSARRecommendationScreen(
                parabolicSARData: parabolicSARData),
          ),
        );
      } else {
  ...
  void _navigateToChartScreen(String symbol) {
    if (_validateSymbol(symbol)) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ParabolicSARChartScreen(symbol: symbol),
        ),
      );
    }
  }
  ...
  void _navigateToStrategiesScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ParabolicSARStrategiesScreen(),
      ),
    );
  }
