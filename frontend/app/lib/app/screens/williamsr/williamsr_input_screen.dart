// williamsr_input_screen.dart
import 'package:flutter/material.dart';
import 'package:flutter_gen/gen_l10n/app_localizations.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'williamsr_recommendation_screen.dart';
import 'williamsr_chart_screen.dart';
import 'williamsr_strategies_screen.dart';

class WilliamsRInputScreen extends StatefulWidget {
  const WilliamsRInputScreen({Key? key}) : super(key: key);

  @override
  _WilliamsRInputScreenState createState() => _WilliamsRInputScreenState();
}

class _WilliamsRInputScreenState extends State<WilliamsRInputScreen> {
  ...
  Future<void> calculateWilliamsR(String symbol) async {
    if (!_validateSymbol(symbol)) {
      return;
    }

    final url = Uri.parse(
        'http://179.42.171.30:12018/companies/$symbol/indicators/williams_r');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final williamsRData = jsonDecode(response.body);
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) =>
                WilliamsRRecommendationScreen(williamsRData: williamsRData),
          ),
        );
      } else {
  ...
  void _navigateToChartScreen(String symbol) {
    if (_validateSymbol(symbol)) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => WilliamsRChartScreen(symbol: symbol),
        ),
      );
    }
  }
  ...
  void _navigateToStrategiesScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => WilliamsRStrategiesScreen(),
      ),
    );
  }
