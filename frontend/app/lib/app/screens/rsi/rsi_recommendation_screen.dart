// rsi_recommendation_screen.dart
import 'package:flutter/material.dart';
import 'package:flutter_gen/gen_l10n/app_localizations.dart';
import 'rsi_help_screen.dart';

class RSIRecommendationScreen extends StatelessWidget {
  final Map<String, dynamic> rsiData;

  const RSIRecommendationScreen({Key? key, required this.rsiData})
      : super(key: key);
              ...
              child: Column(
                ...
                children: <Widget>[
                  if (rsiData.containsKey('RSI'))
                    _buildRSISection(
                      title: 'RSI',
                      content: _buildRSIContent(),
                    ),
                  if (rsiData.containsKey('Recommendation'))
                    _buildRSISection(
                      title: AppLocalizations.of(context)!.recommendation,
                      content: _buildRecommendationContent(
                        context,
                        rsiData['Recommendation'],
                      ),
                    ),
                ],
              ),
  ...
  Widget _buildRSISection({required String title, required Widget content}) {
    return Card(
      elevation: 4,
