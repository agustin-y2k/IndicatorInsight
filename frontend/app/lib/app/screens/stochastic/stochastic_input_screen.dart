...
class StochasticInputScreen extends StatefulWidget {
  ...
  @override
  _StochasticInputScreenState createState() => _StochasticInputScreenState();
}

class _StochasticInputScreenState extends State<StochasticInputScreen> {
  ...
  Future<void> calculateStochastic(String symbol) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final stochasticData = jsonDecode(response.body);
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) =>
                StochasticRecommendationScreen(stochasticData: stochasticData),
          ),
        );
      } else {
  ...
  void _navigateToChartScreen(String symbol) {
    if (_validateSymbol(symbol)) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => StochasticChartScreen(symbol: symbol),
        ),
      );
    }
  }
  ...
  void _navigateToStrategiesScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => StochasticStrategiesScreen(),
      ),
    );
  }
