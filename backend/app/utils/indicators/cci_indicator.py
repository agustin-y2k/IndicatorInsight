...
def calculate_cci(company_data):
    try:
        period=20
...
def cci_recommendation(cci_values, data_df):
    ...
    if current_cci > (100 + safety_margin):
        return handle_high_cci(current_cci, previous_cci, cci_trend, strong_threshold, safety_margin)
    elif current_cci < (-100 - safety_margin):
        return handle_low_cci(current_cci, previous_cci, cci_trend, strong_threshold, safety_margin)
    else:
        return handle_neutral_cci(cci_values, data_df, cci_trend)

def handle_high_cci(current_cci, previous_cci, cci_trend, strong_threshold, safety_margin):
...
def handle_low_cci(current_cci, previous_cci, cci_trend, strong_threshold, safety_margin):
    if current_cci < -strong_threshold or (previous_cci >= (-100 - safety_margin) and cci_trend < 0):
        return Recommendation.STRONG_BUY if current_cci < -strong_threshold else Recommendation.BUY

def handle_neutral_cci(cci_values, data_df, cci_trend):
